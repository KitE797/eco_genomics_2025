...
# Note: We are below the general rule of thumb recommendation of 20M reads per sample
...
```{r}
head(apply(countsTableRound,2,mean)) # 2 in the apply function does the action across columns
head(apply(countsTableRound,1,mean)) # 1 in the apply function does the action across rows
```

```{r}
hist(apply(countsTableRound,1,mean),ylim=c(0,1000),xlim=c(0,3000),breaks=50000)
```

```{r}
# Zoomed in plot:
hist(apply(countsTableRound,1,mean),ylim=c(0,10),breaks=100)
```
...
p
```
Question: What is the relationship between log-fold change and the significance of differential gene expression?
...
ggplot(res_df, aes(x = log2FoldChange, y = -log10(padj), color = significance)) +
  ...
  scale_color_manual(values = c("Up" = "darkblue", "Down" = "darkred", "Not Sig" = "grey")) +
...
Trying to make a scatterplot for log-fold change in G1 and log-fold change in G2, etc
```{r}
# to get log-fold change: 
# Make a dataframe
res_df1 <- as.data.frame(resG1_CvT)
res_df2 <- as.data.frame(resG2_CvT)
res_df3 <- as.data.frame(resG3_CvT)
res_df4 <- as.data.frame(resG4_CvT)

# Remove NAs
res_df1 <- res_df1[!is.na(res_df1$padj), ]
res_df2 <- res_df2[!is.na(res_df2$padj), ]
res_df3 <- res_df3[!is.na(res_df3$padj), ]
res_df4 <- res_df4[!is.na(res_df4$padj), ]

# Add significance column for color mapping
res_df1$significance <- "Not Sig"
res_df1$significance[res_df1$padj < 0.05 & res_df1$log2FoldChange > 0] <- "Up"
res_df1$significance[res_df1$padj < 0.05 & res_df1$log2FoldChange < 0] <- "Down"

res_df2$significance <- "Not Sig"
res_df2$significance[res_df2$padj < 0.05 & res_df2$log2FoldChange > 0] <- "Up"
res_df2$significance[res_df2$padj < 0.05 & res_df2$log2FoldChange < 0] <- "Down"

res_df3$significance <- "Not Sig"
res_df3$significance[res_df3$padj < 0.05 & res_df3$log2FoldChange > 0] <- "Up"
res_df3$significance[res_df3$padj < 0.05 & res_df3$log2FoldChange < 0] <- "Down"

res_df4$significance <- "Not Sig"
res_df4$significance[res_df4$padj < 0.05 & res_df4$log2FoldChange > 0] <- "Up"
res_df4$significance[res_df4$padj < 0.05 & res_df4$log2FoldChange < 0] <- "Down"
df <- tibble::rownames_to_column(df, "VALUE")



big_df <- merge(res_df1, res_df2, by=)
