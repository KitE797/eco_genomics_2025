...
Importing counts matrix, sample description table
```{r}
...
Creating DESeq object, filtering
```{r}
...
Importing the phenotypic data, creating a subset matrix from filtering
```{r}
...
# calculate variance for each gene. 2 is for 'look across columns'
gene_vars <- apply(norm.counts, 2, var)

# keep top 50% most variable genes (adjust proportion as needed)
# if we didn't have an even number, we would want to round down
...
#0.8 is recommended, which is why the yintercept is set to that
...
# set maxblocksize to greater than the number of genes to run network construction all at once
...
# "use='p'": using pearson's correlation
...
MEcyan_summary <- summarySE(module_eigengene.metadata, measurevar="MEcyan", groupvars=c("generation","line"))

#Plot a line interaction plot of a particular module eigengene
ggplot(MEcyan_summary, aes(x=as.factor(generation), y=MEcyan, color=line, fill = line, shape = line)) +
  geom_point(size=5, stroke = 1.5 ) +
  geom_errorbar(aes(ymin=MEcyan-se, ymax=MEcyan+se), width=.15) +
  geom_line(aes(color=line, group=line, linetype = line)) +
  scale_color_manual(values = c('blue', "darkcyan")) +
  scale_shape_manual(values=c(21,23), labels = c("Ambient","22 at G1"))+
  scale_fill_manual(values=c('blue',"darkcyan"), labels = c("Ambient","22 at G1"))+
  xlab("Generation") +
  theme_bw() +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 4))+
  theme(text = element_text(size = 20)) +
  theme(panel.grid.minor.y = element_blank(), plot.margin = margin(0,6,0,6))
```
```{r}
...
geneIDs_cyan <- module.gene.mapping %>% filter(`bwnet$colors` == 'cyan') %>% rownames()

# Filter annotation for module genes -- change the name here
module_GO <- tonsa_go[tonsa_go$geneID %in% geneIDs_cyan, ]
