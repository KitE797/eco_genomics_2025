...
countsTableRound <- round(countsTable1) # bc DESeq2 doesn't like decimals (and Salmon outputs data with decimals)
...
dds <- dds[rowSums(counts(dds) >= 12) >= 12,]
nrow(dds) 
...
```{r}
resG1_AAHHvAAAA <- results(dds, contrast=c("group","AAHH","AAAA"), alpha = 0.05) #alpha = significance value 
summary(resG1_AAHHvAAAA)

resG1_HHAAvAAAA <- results(dds, contrast=c("group","HHAA","AAAA"), alpha = 0.05) #alpha = significance value 
summary(resG1_HHAAvAAAA)

resG1_HHHHvAAAA <- results(dds, contrast=c("group","HHHH","AAAA"), alpha = 0.05) #alpha = significance value 
summary(resG1_HHHHvAAAA)
```

Reorder by significance
```{r}
resG1_HHHHvAAAA <- resG1_HHHHvAAAA[order(resG1_HHHHvAAAA$padj),]
head(resG1_HHHHvAAAA)
```
...
d <-plotCounts(dds, gene="TRINITY_DN147187_c0_g12", intgroup = (c("treatment","generation")), returnData=TRUE)
d
...
p <- ggplot(d, aes(x = treatment, y = count, color = treatment)) +
  theme_minimal() + theme(text = element_text(size=20), panel.grid.major=element_line(colour="grey"))
p <- p + geom_point(position=position_jitter(w=0.2,h=0), size=3, aes(shape = treatment)) 
p <- p + stat_summary(fun = mean, geom = "point", size=5, alpha=0.7) 
p <- p + stat_summary(fun = mean, geom = "line", size = 1.5, aes(group = line), alpha = 0.8)
p <- p + scale_color_manual(values = c("control" = "blue", "treatment" = "red"))
p
